...
Vue.filter('formatFromToDate', function (startDate, endDate, atTranslation, toTranslation) {
    ...
    return `${getFormatDate(startDate)} ${atTranslation} ${format(startDate * 1000, 'H:mm', getOptions())} 
              - ${getFormatDate(endDate)} ${toTranslation} ${format(endDate * 1000, 'H:mm', getOptions())} `
});
