...
const unsubscribe = requireModel('unsubscribe/notifications');

const schemaUnsubscribeNotifications = {
...
module.exports = function (router) {
    ...
    router.post('/', apiLimiter, asyncMiddleware(async (req, res) => {
        let request = await validation.validateRequest(req, schemaUnsubscribeNotifications);
        logger.info(`Unsubscribe to new notifications ${request.email}`, req);
