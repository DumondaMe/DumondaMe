...
const harvestingUser = require('./harvestingUser/index');
const harvestingUserResponse = require('./harvestingUser/response');
...
const search = async function (query, language, userId) {
    let result = await user.searchCommand(query, language, userId, 0, NUMBER_OF_ELEMENTS + 1)
        .send([commitment.searchCommand(query, language, userId, 0, NUMBER_OF_ELEMENTS + 1).getCommand(),
            ...
            harvestingUser.searchCommand(query, language, userId, 0, NUMBER_OF_ELEMENTS + 1).getCommand()]);

    let hasMoreUsers = moreSearchResult.getHasMoreResults(result[3], NUMBER_OF_ELEMENTS);
    let hasMoreHarvestingUsers = moreSearchResult.getHasMoreResults(result[2], NUMBER_OF_ELEMENTS);
    ...
    return {
        users: await userResponse.getResponse(result[3], userId), hasMoreUsers,
        harvestingUsers: await harvestingUserResponse.getResponse(result[2], userId), hasMoreHarvestingUsers,
