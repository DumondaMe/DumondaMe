...
describe('Get activity feed with created text answers for a user', function () {
    ...
    it('Created text answer of a user', async function () {
    ...
    it('Created text answer for logged in user', async function () {
        await dbDsl.sendToDb();
        await requestHandler.login(users.validUser);
        let res = await requestHandler.get('/api/user/profile/activity',
