...
var getPinwall = function (userId, request) {
    return db.cypher().match("(:User {userId: {userId}})-[:IS_CONTACT]->(contact:User)" +
        "-[:RECOMMENDS]->(rec:Recommendation)-[:RECOMMENDS]->(page:Page)")
        .with("contact, rec, page")
        ...
        "page.label AS label, page.title AS title, page.pageId AS pageId, v.profile AS profileVisible, v.image AS imageVisible")
