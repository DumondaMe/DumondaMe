...
const createCommitment= function (answerId, data, addresses) {
    ...
    dbConnectionHandling.getCommands().push(db.cypher().match("(user:User {userId: {adminId}})")
        ...
        created: {created}, topic: {topic}, answerId: {answerId}, website: {website}}) 
        ...
        .end({
            answerId: answerId, adminId: data.adminId, title: data.title, description: data.description, language: data.language, topic: data.topic, created: data.created,
