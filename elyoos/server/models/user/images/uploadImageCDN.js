    ...
    return gm.gm(originalFilePath).sizeAsync()
        .then(function (size) {
            ...
            if (previewHeight > maxPreviewHeight) {
                ...
                return gm.gm(originalFilePath).resize(null, maxPreviewHeight).quality(80).background('#FFFFFF').flatten()
                    .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(preview.name);
            }
            return gm.gm(originalFilePath).resize(widthPreview).quality(80).background('#FFFFFF').flatten()
                .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(preview.name);
        })
        .then(function () {
            if (previewHeight === maxPreviewHeight) {
                return gm.gm(originalFilePath).resize(null, maxPreviewHeight).quality(86).background('#FFFFFF').flatten()
                    .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(normal.name);
            } else if(sizeOriginal.width > 600) {
                return gm.gm(originalFilePath).resize(600).quality(86).background('#FFFFFF').flatten()
                    .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(normal.name);
            } else if(sizeOriginal.width > 380) {
                return gm.gm(originalFilePath).resize(sizeOriginal.width).quality(86).background('#FFFFFF').flatten()
                    .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(normal.name);
            }
            return gm.gm(originalFilePath).resize(380).quality(86).background('#FFFFFF').flatten()
                .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(normal.name);
        })
        .then(function () {
            return gm.gm(originalFilePath).resize(64).quality(86).background('#FFFFFF').flatten()
                .unsharp(2 + sigma, sigma, amount, threshold).noProfile().writeAsync(thumbnail.name);
        })
        .then(function () {
