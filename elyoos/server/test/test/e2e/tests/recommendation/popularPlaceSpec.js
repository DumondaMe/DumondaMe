'use strict';

let users = require('elyoos-server-test-util').user;
let dbDsl = require('elyoos-server-test-util').dbDSL;
let requestHandler = require('elyoos-server-test-util').requestHandler;

describe('Integration Tests for getting popular address recommendations', function () {

    let requestAgent;

    beforeEach(function () {
        return dbDsl.init(8);
    });

    afterEach(function () {
        return requestHandler.logout();
    });

    it('Getting most popular place recommendations for a area - Return 200', function () {

        dbDsl.createPrivacyNoContact(null, {profile: true, image: true, profileData: true, contacts: true, pinwall: true});
        dbDsl.createContactConnection('1', '2');
        dbDsl.createContactConnection('1', '3');

        dbDsl.createGenericPage('1', '1', ['de'], ['health', 'personalDevelopment'], 5072, 'Test1',
            [{address: 'Zuerich', lat: 47.376887, lng: 8.541694, addressId: '1'}, {address: 'Zuerich Paradeplatz', lat: 47.369890, lng: 8.539127, addressId: '2'}]);
        dbDsl.createGenericPage('2', '1', ['de'], ['health', 'personalDevelopment'], 5073, 'Test2',
            [{address: 'Urdorf', lat: 47.386707, lng: 8.420693, addressId: '3'}, {address: 'Urdorf2', lat: 47.38, lng: 8.42, addressId: '4'}]);
        dbDsl.createGenericPage('3', '1', ['de'], ['health', 'personalDevelopment'], 5074, 'Test3',
            [{address: 'Altstetten', lat: 47.388237, lng: 8.483051, addressId: '5'}]);
        dbDsl.createGenericPage('4', '1', ['de'], ['health', 'personalDevelopment'], 5075, 'Test4',
            [{address: 'Bern', lat: 46.947974, lng: 7.447447, addressId: '6'}]);
        dbDsl.createGenericPage('5', '1', ['de'], ['health', 'personalDevelopment'], 5076, 'Test5',
            [{address: 'Genf', lat: 46.204391, lng: 6.143158, addressId: '7'}]);

        dbDsl.crateRecommendationsForPage('2', [{userId: '2', created: 500}, {userId: '3', created: 501}]);
        dbDsl.crateRecommendationsForPage('3', [{userId: '3', created: 502}, {userId: '4', created: 503}, {userId: '5', created: 504}]);
        dbDsl.crateRecommendationsForPage('4', [{userId: '3', created: 504}, {userId: '4', created: 505},
            {userId: '5', created: 506}, {userId: '6', created: 507}]);
        dbDsl.crateRecommendationsForPage('5', [{userId: '3', created: 508}, {userId: '4', created: 509}, {userId: '7', created: 510}]);


        return dbDsl.sendToDb().then(function () {
            return requestHandler.login(users.validUser).then(function (agent) {
                requestAgent = agent;
                return requestHandler.getWithData('/api/recommendation/popularAddress', {
                    skip: '0',
                    maxItems: 10,
                    centerLat: 47.376887,
                    centerLng: 8.541694,
                    radius: 20
                }, requestAgent);
            }).then(function (res) {
                res.status.should.equal(200);

                res.body.recommendations.length.should.equals(3);
                res.body.recommendations[0].pageId.should.equals('3');
                res.body.recommendations[0].url.should.equals('pages/3/thumbnail.jpg');
                res.body.recommendations[0].title.should.equals('Test3');
                res.body.recommendations[0].addresses.length.should.equals(1);
                res.body.recommendations[0].addresses[0].address.should.equals('Altstetten');
                res.body.recommendations[0].addresses[0].latitude.should.equals(47.388237);
                res.body.recommendations[0].addresses[0].longitude.should.equals(8.483051);
                res.body.recommendations[0].topic.length.should.equals(2);
                res.body.recommendations[0].topic[0].should.equals('health');
                res.body.recommendations[0].topic[1].should.equals('personalDevelopment');
                res.body.recommendations[0].numberOfRecommendations.should.equals(3);

                res.body.recommendations[1].pageId.should.equals('2');
                res.body.recommendations[1].title.should.equals('Test2');
                res.body.recommendations[1].url.should.equals('pages/2/thumbnail.jpg');
                res.body.recommendations[1].addresses.length.should.equals(2);
                res.body.recommendations[1].addresses[0].address.should.equals('Urdorf');
                res.body.recommendations[1].addresses[0].latitude.should.equals(47.386707);
                res.body.recommendations[1].addresses[0].longitude.should.equals(8.420693);
                res.body.recommendations[1].addresses[1].address.should.equals('Urdorf2');
                res.body.recommendations[1].addresses[1].latitude.should.equals(47.38);
                res.body.recommendations[1].addresses[1].longitude.should.equals(8.42);
                res.body.recommendations[1].topic.length.should.equals(2);
                res.body.recommendations[1].topic[0].should.equals('health');
                res.body.recommendations[1].topic[1].should.equals('personalDevelopment');
                res.body.recommendations[1].numberOfRecommendations.should.equals(2);

                res.body.recommendations[2].pageId.should.equals('1');
                res.body.recommendations[2].title.should.equals('Test1');
                res.body.recommendations[2].url.should.equals('pages/1/thumbnail.jpg');
                res.body.recommendations[2].addresses.length.should.equals(2);
                res.body.recommendations[2].addresses[0].address.should.equals('Zuerich');
                res.body.recommendations[2].addresses[0].latitude.should.equals(47.376887);
                res.body.recommendations[2].addresses[0].longitude.should.equals(8.541694);
                res.body.recommendations[2].addresses[1].address.should.equals('Zuerich Paradeplatz');
                res.body.recommendations[2].addresses[1].latitude.should.equals(47.369890);
                res.body.recommendations[2].addresses[1].longitude.should.equals(8.539127);
                res.body.recommendations[2].topic.length.should.equals(2);
                res.body.recommendations[2].topic[0].should.equals('health');
                res.body.recommendations[2].topic[1].should.equals('personalDevelopment');
                res.body.recommendations[2].numberOfRecommendations.should.equals(0);
            });
        });
    });
});
